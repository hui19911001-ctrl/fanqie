...
export default function Timer({
        ...
        if (now < targetEndTime) {
          ...
          if (!isOvertime) {
            ...
            if (mode === 'focus') {
              ...
              showToast?.("休息时间到！已进入额外放松时间 ☕", "info");
            }
          }
  ...
  const toggleTimer = () => {
    ...
    if (isOvertime) {
      if (mode === 'focus') {
        ...
        // In overtime break: play/pause directly finishes rest and switches to focus!
        handleSettleBreakOvertime();
      }
    } else {
  ...
  const handleSettleBreakOvertime = () => {
    playClick(settings.soundVolume);
    setIsOvertime(false);
    ...
    // Call completion callback for break completion
    if (onPomodoroComplete) {
      onPomodoroComplete(mode);
    }

    showToast?.("休息段落圆满结束！开始新的意蕴专注 🌿", "success");
    ...
    <div id="pomodoro-timer-card" className="bg-white rounded-3xl p-6 md:p-8 border border-morandi-sand shadow-sm flex flex-col items-center justify-between text-center relative overflow-hidden transition-all-custom">
        ...
        <div className="absolute flex flex-col items-center justify-center w-56">
          {isOvertimeChoiceOpen ? (
            ...
            <button
              ...
              title={isOvertime ? (mode === 'focus' ? "点击完成结算专注" : "点击结束休息并开启专注") : "点击时间可直接键盘打字修改时间"}
            >
              <div className="flex items-center space-x-0.5">
                {isOvertime && (
                  <span className={`font-mono text-3xl md:text-3xl font-medium tracking-tighter animate-pulse mr-1 ${
                    mode === 'focus' ? 'text-amber-600' : 'text-morandi-sage'
                  }`}>
                    +
                  </span>
                )}
                <span className={`font-mono text-5xl md:text-6xl font-light tracking-tighter transition-colors duration-200 ${
                  isOvertime 
                    ? (mode === 'focus' ? 'text-amber-600 font-medium' : 'text-morandi-sage font-medium')
              ...
              <span className="text-[8px] tracking-wide text-morandi-charcoal/30 group-hover/time:text-morandi-sage uppercase mt-1 transition-all duration-200 flex items-center space-x-1 select-none whitespace-nowrap">
                {isOvertime ? (
                  mode === 'focus' ? (
                    ...
                    <span className="text-morandi-sage animate-pulse">🌿 休息已经额外超出 (点击结束)</span>
                  )
                ) : (
          ...
          <span className="text-[10px] tracking-widest text-morandi-charcoal/40 uppercase mt-2 select-none">
            {isOvertime ? (mode === 'focus' ? 'Overtime Focus' : 'Overtime Relax') : mode === 'focus' ? 'Focus Period' : 'Relax Chill'}
          </span>
        </div>
      ...
      {isOvertime ? (
        <motion.button
          ...
          className={`mt-3 px-6 py-2.5 rounded-full text-sm font-semibold text-white shadow-md transition-all-custom flex items-center space-x-1.5 z-10 select-none cursor-pointer border ${
            mode === 'focus'
              ? 'bg-amber-500 hover:bg-amber-600 border-amber-600/30'
              : 'bg-morandi-sage hover:bg-morandi-sage-hover border-morandi-sage-hover/30'
          }`}
        >
          <Check className="w-4 h-4" />
          <span>
            {mode === 'focus'
              ? `点击完成专注（+${Math.round(overtimeSeconds / 60)}分钟） 🎯`
              : `结束休息，开启专注 ☕`
            }
          </span>
        </motion.button>
        ...
        mode === 'focus' && timeLeft < totalDuration ? (
          ...
          (mode === 'break' || mode === 'longBreak') && timeLeft < totalDuration && (
            <motion.button
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 10 }}
              id="early-complete-rest-btn"
              onClick={handleSettleBreakOvertime}
              className="mt-3 px-4 py-1.5 rounded-full text-xs font-semibold border border-morandi-sage text-morandi-sage hover:bg-morandi-sage hover:text-white transition-all-custom flex items-center space-x-1 shadow-3xs z-10 select-none cursor-pointer"
            >
              <Check className="w-3.5 h-3.5" />
              <span>提前结束休息并开始专注 🌿</span>
            </motion.button>
          )
        )
      )}
